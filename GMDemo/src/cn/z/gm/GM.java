...
//			ISOException.throwIt(ISO7816.SW_SECURITY_STATUS_NOT_SATISFIED);
	...
	public void GEN_SM2_KEYPAIR(APDU apdu)throws CardRuntimeException
	{
		...
		short reslen = 0;
		short publen = 0;
		SM2PublicKey SM2pubkeytmp = null;
...
//			ISOException.throwIt(ISO7816.SW_SECURITY_STATUS_NOT_SATISFIED);
		...
		if(p1 == 0x01){
			...
			SM2pubkeytmp = SM2pubkey1;
			
			reslen = SM2pubkey1.getW(buf, ISO7816.OFFSET_CDATA);
			reslen = SM2prikey1.getS(buf, (short)(ISO7816.OFFSET_CDATA+reslen));
		}
		else if(p1 == 0x02){
			GM_KeyPair.genSM2KeyPair();
			SM2prikey2 = (SM2PrivateKey) GM_KeyPair.getPrivate();
			SM2pubkey2 = (SM2PublicKey) GM_KeyPair.getPublic();
			SM2pubkeytmp = SM2pubkey2;
			reslen = SM2pubkey2.getA(buf, ISO7816.OFFSET_CDATA);
		}
		else if(p1 == 0x03){
			GM_KeyPair.genSM2KeyPair();
			SM2prikey3 = (SM2PrivateKey) GM_KeyPair.getPrivate();
			SM2pubkey3 = (SM2PublicKey) GM_KeyPair.getPublic();
			SM2pubkeytmp = SM2pubkey3;
			reslen = SM2pubkey3.getB(buf, ISO7816.OFFSET_CDATA);
		}
		else if(p1 == 0x04){
			GM_KeyPair.genSM2KeyPair();
			SM2prikey4 = (SM2PrivateKey) GM_KeyPair.getPrivate();
			SM2pubkey4 = (SM2PublicKey) GM_KeyPair.getPublic();
			SM2pubkeytmp = SM2pubkey4;
			reslen = SM2pubkey4.getG(buf, ISO7816.OFFSET_CDATA);
		}
		else if(p1 == 0x05){
			GM_KeyPair.genSM2KeyPair();
			SM2prikey5 = (SM2PrivateKey) GM_KeyPair.getPrivate();
			SM2pubkey5 = (SM2PublicKey) GM_KeyPair.getPublic();
			SM2pubkeytmp = SM2pubkey5;
			reslen = SM2pubkey4.getW(buf, ISO7816.OFFSET_CDATA);
		}
		//拷贝公钥到存储区域
		SM2pubkeytmp.getW(TransPubKey,  (short)((p1-1)*65 + 1));
		TransPubKey[(short)((p1-1)*65)] = p1;
